...
#tmux send-keys 'docker-compose -f node/testnet/docker-compose.yml up' C-m
sleep 5
tmux split-window -h
...
#tmux send-keys 'docker cp node/testnet/fill_user_account.sh qtum_regtest:.; docker exec qtum_regtest /bin/sh -c ./fill_user_account.sh' C-m
